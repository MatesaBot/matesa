...
def createApp (connect):
    appname = "matesa" + str(time() * 1000)[-4:].replace(".", "") + str(random.randint(0,500))
    try:
...
async def botlog (String, Api, Hash):
    ...
    KanalId = await Client(CreateChannelRequest(
        title='MatesaUserBot BotLog',
        about=LANG['AUTO_BOTLOG'],
        megagroup=True
    ))
    KanalId = KanalId.chats[0].id

    Photo = await Client.upload_file(file='matesalogo.jpg')
...
if __name__ == "__main__":
    ...
    config['DEFAULT_BIO'] = "@MatesaUserBot"
    ...
    config['TZ'] = TZ
    config['TZ_NUMBER'] = "1"
    config['UPSTREAM_REPO_URL'] = "https://github.com/matesa/MatesaUserBot"
    config['WARN_LIMIT'] = "3"
